  ...
  on_supported_os.each do |os, os_facts|
    context "on #{os}" do
      ...
      it {
        ...
        is_expected.to contain_file('docker-compose-pihole')
          .with(ensure: 'file')
          .with(owner: 'root')
          .with(group: 'root')
          .with(mode: '0644')
          .with(path: '/opt/docker-compose/dnsserver/docker-compose.yml')
          .with(require: 'File[/opt/docker-compose/dnsserver/]')
        is_expected.to contain_docker_compose('pihole')
          .with(ensure: 'present')
          .with(compose_files: ['/opt/docker-compose/dnsserver/docker-compose.yml'])
          .with(subscribe: 'File[docker-compose-pihole]')
        is_expected.to contain_firewall('100 allow 1080 to 192.168.0.0/16')
      ...
      context "with no param set" do
        it {
          verify_contents(catalogue, "docker-compose-pihole", [
            "version: '3'",
            "services:",
            "  pihole:",
